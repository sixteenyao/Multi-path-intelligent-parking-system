...
// ======================== 车轨迹跟踪 ========================
struct CarTrack{int id;DetectBox box;int in_cnt,out_cnt;bool inside;CarTrack():id(0),in_cnt(0),out_cnt(0),inside(false){}};
struct CarTracker{
    std::vector<CarTrack> tracks;int next_id=1;
    ...
    void update(const std::vector<DetectBox>&dets,const std::vector<std::pair<int,int>>&mask,const cv::Mat&frame,
                int cam_id,std::vector<Event>&events){
        events.clear();int n=tracks.size(),m=dets.size();
        std::vector<bool>used(m,false);
        for(int i=0;i<n;i++){float best=0.3f;int bj=-1;
            for(int j=0;j<m;j++){if(used[j])continue;float i=iou(tracks[i].box,dets[j]);if(i>best){best=i;bj=j;}}
            if(bj>=0){used[bj]=true;tracks[i].box=dets[bj];}else{tracks[i].id=-1;}}
        ...
        tracks.erase(std::remove_if(tracks.begin(),tracks.end(),[](CarTrack&t){return t.id<0;}),tracks.end());
        for(int j=0;j<m;j++)if(!used[j]){CarTrack t;t.id=next_id++;t.box=dets[j];
            int cx=(dets[j].left+dets[j].right)/2,cy=(dets[j].top+dets[j].bottom)/2;
            if(cam_id!=0){cx=xoo+cx*dww/frame.cols;cy=yoo+cy*dhh/frame.rows;}
            t.inside=pt_in_poly(cx,cy,mask);t.in_cnt=t.inside?99:0;t.out_cnt=t.inside?0:99;
            tracks.push_back(t);}

        for(auto&t:tracks){int cx=(t.box.left+t.box.right)/2,cy=(t.box.top+t.box.bottom)/2;
            if(cam_id!=0){cx=xoo+cx*dww/frame.cols;cy=yoo+cy*dhh/frame.rows;} // A跳过转换
            bool now_in=pt_in_poly(cx,cy,mask);
            if(now_in){t.in_cnt++;t.out_cnt=0;}else{t.out_cnt++;t.in_cnt=0;}
            if(t.in_cnt>=2&&!t.inside){t.inside=true;events.push_back({t.id,true});}
            if(t.out_cnt>=2&&t.inside){t.inside=false;events.push_back({t.id,false});}}
    }
};
...
int main(){
    ...
    int prev_inside[4]={-1,-1,-1,-1}; // 上轮蒙版内车数, 用于检测进出
        ...
        {cv::Mat t;int c=bg;bg=(bg+1)%4;
         if(true){ // 始终读所有通道, 保证后台检测全覆盖
            ...
            if(!t.empty()){
                ...
                if(!masks[c].empty()){
                    bg_det_skip[c]=(bg_det_skip[c]+1)%10;
                    if(bg_det_skip[c]==0){
                        ...
                        chan_per[c]=pc;
                        {int ni=0;for(auto&b:bbx){int cxx=(b.left+b.right)/2,cyy=(b.top+b.bottom)/2;
                             if(c!=0){cxx=xob+cxx*dwb/t.cols;cyy=yob+cyy*dhb/t.rows;}
                             if(pt_in_poly(cxx,cyy,masks[c]))ni++;}
                         if(prev_inside[c]>=0){int d=ni-prev_inside[c];if(d>0)printf("[event] %c区 驶入%d辆\n",'A'+c,d);if(d<0)printf("[event] %c区 驶出%d辆\n",'A'+c,-d);}
                         prev_inside[c]=ni;
                         printf("[det] %c区 %d车 内%d\n",'A'+c,(int)bbx.size(),ni);}
                        chan_car[c]=cc;
