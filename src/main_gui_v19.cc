...
int main(){
    ...
    masks[2]={{{469,68},{619,105},{638,147},{798,168},{1264,1079},{647,1079}},{{714,0},{1046,246},{1065,305},{1681,561},{1714,424},{920,0}}}; // C双蒙版
    ...
    while(1){
        ...
        {for(int c=0;c<4;c++){
            if(state==VIDEO&&c==cam)continue; // VIDEO模式由前景读帧, 后台跳过
            ...
            // 检测: 每4轮检一次(降低CPU占用)
            if(!masks[c].empty()){
                bg_det_skip[c]=(bg_det_skip[c]+1)%4;
                if(bg_det_skip[c]==0){
