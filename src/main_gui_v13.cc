...
int main(){
    ...
    int chan_car[4]={0}, chan_cap[4]={200,120,24,40}; // 每通道车辆数/容量
    char chan_info[4][32]={}, chan_status[4][16]={};
    int bg_det_skip[4]={}, bg_car_acc[4]={}, bg_acc_cnt[4]={}; // 后台检测累加器
        ...
        {cv::Mat t;int c=bg;bg=(bg+1)%4;
         if(c!=cam||state==MAIN){
            ...
            if(!t.empty()){
                ...
                if(state==MAIN && !masks[c].empty()){
                    bg_det_skip[c]=(bg_det_skip[c]+1)%30;
                    if(bg_det_skip[c]==0){
