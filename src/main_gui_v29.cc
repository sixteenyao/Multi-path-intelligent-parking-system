...
// 长文本自动换行(每行45中文字宽, 仅第一行带[AI]前缀)
static void llm_add_wrapped(std::vector<std::string>&log,const char*text){
    const int MAXW=45*2; // 45中文=90宽度单位(中文2/ASCII1)
    std::string line="[AI] ";int line_w=0;
    for(const char*p=text;*p;){
        int clen=1;bool ascii=((unsigned char)*p<128);
        if(!ascii){if((*p&0xE0)==0xC0)clen=2;else if((*p&0xF0)==0xE0)clen=3;else clen=4;}
        int cw=ascii?1:2;
        if(line_w+cw>MAXW){log.push_back(line);line="";line_w=0;}
...
static void llm_ask(const char*q){
    ...
    if(fork()==0){execl("/bin/sh","sh","-c","/root/llm_async.sh",NULL);_exit(0);}
}
// 检查LLM回复(每帧调用, 非阻塞)
    ...
    while(1){
        ...
        for(int k=0;k<3;k++)if(kbd_fds[k]>=0){input_event ev;while(read(kbd_fds[k],&ev,sizeof(ev))>0){
            if(ev.type==EV_KEY&&ev.value==1){
                if(ev.code==KEY_ENTER){llm_log.push_back("> "+input_buf);llm_ask(input_buf.c_str());input_buf.clear();}
        ...
        {fd_set rfds;FD_ZERO(&rfds);FD_SET(tcp_fd,&rfds);timeval tv={0,0};
         ...
         if(select((tcp_cli>=0?tcp_cli:tcp_fd)+1,&rfds,NULL,NULL,&tv)>0){
             ...
             if(tcp_cli>=0&&FD_ISSET(tcp_cli,&rfds)){char tcp_buf[256];int n=read(tcp_cli,tcp_buf,255);
                 if(n>0){tcp_buf[n]=0;for(char*p=tcp_buf;*p;p++){if(*p=='\n'){llm_log.push_back("> "+input_buf);llm_ask(input_buf.c_str());input_buf.clear();}else if(*p==127||*p==8){if(!input_buf.empty())input_buf.pop_back();}else if(*p>=32)input_buf+=*p;}}
        ...
        if(state==MAIN){
            ...
            if(touch.clicked){
                ...
                if(tx>=20&&tx<=110&&ty>=20&&ty<=70){state=MAIN;cam=-1;empty_cnt=0;}
            }
