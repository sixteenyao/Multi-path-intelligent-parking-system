    ...
    while(1){
        ...
        if(state==MAIN){
            ...
            if(touch.clicked){for(int i=0;i<5;i++)if(tx>=btn[i].x&&tx<btn[i].x+bw&&ty>=btn[i].y&&ty<btn[i].y+bh){
                cam_mode=(i==4); cam=(i==4?0:i); state=VIDEO;empty_cnt=0;
                cnt_car_acc=cnt_per_acc=avg_cnt=0;
                if(cam_mode){cap_cam.release();cap_cam.open("rtsp://192.168.131.62:1935");printf("[cam] %d\n",cap_cam.isOpened());}
