...
void bL(int bk,int lx,int ly,int rw,int rh,C4 c,int t=3){for(int y=0;y<rh;y++)for(int x=0;x<rw;x++)if(x<t||x>=rw-t||y<t||y>=rh-t)pL(bk,lx+x,ly+y,c);}
	void rot_write(int bk,cv::Mat&img,int lw,int lh){
	    ...
	    const int S=3; // 步长3, ~23万像素, 画质几乎无差异
...
int main(){
    ...
    masks[1]={{{0,349},{685,265},{1919,361},{1919,1079},{0,1079}}}; // B
    masks[2]={{{469,68},{619,105},{638,147},{798,168},{1264,1079},{647,1079}},{{714,0},{1046,246},{1265,305},{1681,561},{1714,424},{920,0}}}; // C双蒙版
    masks[3]={{{0,493},{1333,448},{1919,879},{1919,1079},{0,1079}}}; // D
    ...
    while(1){
        ...
        {for(int c=0;c<4;c++){
            if(state==VIDEO&&c==cam)continue; // VIDEO模式只读其他通道, 当前通道由前景处理
            ...
            // 检测: 每4轮检一次(降低CPU占用)
            if(!masks[c].empty()){
                bg_det_skip[c]=(bg_det_skip[c]+1)%6;
                if(bg_det_skip[c]==0){
